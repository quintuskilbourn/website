...
Microsoft Azure is currently the only supported cloud provider, as it offers end-to-end working TEE attestations.
...
For Azure, we recommend the [Standard_EC32es_v6](https://learn.microsoft.com/en-us/azure/virtual-machines/ecesv5-ecedsv5-series#ecesv6-series-1) instance type.
You'll need to request a quota for "Standard ECesv6 Family vCPUs" (32 vCPUs).
...
Bare metal hosting is not yet supported. We're working with providers including Google and OpenMetal to resolve remaining issues with measurements, attestations, and BIOS upgrades. We expect bare-metal hosting to be available in the future.
