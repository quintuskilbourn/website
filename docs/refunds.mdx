...
Users, wallets, apps, and searchers who privately submit transactions or bundles to BuilderNet receive refunds based on the value their orderflow adds. The goal of the refund rule is to minimize the amount that each transaction or bundle pays to land onchain. At a high level:
...
Refund data is available through the [BuilderNet APIs](./api.mdx#Refunds), [BuilderNet dashboard](https://dune.com/flashbots/buildernet), and the [Flashbots Protect dashboard](https://dune.com/flashbots/flashbots-protect).
